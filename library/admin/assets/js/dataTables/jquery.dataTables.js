...
(/** @lends <global> */function( $ ) {
	...
	function _fnBuildAjax( oSettings, data, fn )
	{
		...
		var baseAjax = {
			"data": data,
			"success": function (j) {
				var error = j.error || j.sError;
				if ( error ) {
					oSettings.oApi._fnLog( oSettings, 0, error );
				}
	
				oSettings.j = j;
				_fnCallbackFire( oSettings, null, 'xhr', [oSettings, j] );
				fn( j );
			},
			"dataType": "j",
			"cache": false,
			...
			"error": function (xhr, error, thrown) {
				...
				if ( error == "parsererror" ) {
					log( oSettings, 0, 'Invalid J response', 1 );
				}
				else {
	...
	function _fnAjaxUpdate( oSettings )
	{
		if ( oSettings.bAjaxDataGet )
		{
			...
			_fnBuildAjax( oSettings, aoData, function(j) {
				_fnAjaxUpdateDraw( oSettings, j );
			}, oSettings );
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @param {object} j j data return from the server.
	 *  @param {string} j.sEcho Tracking flag for DataTables to match requests
	 *  @param {int} j.iTotalRecords Number of records in the data set, not accounting for filtering
	 *  @param {int} j.iTotalDisplayRecords Number of records in the data set, accounting for filtering
	 *  @param {array} j.aaData The data to display on this page
	 *  @param {string} [j.sColumns] Column ordering (sName, comma separated)
	 *  @memberof DataTable#oApi
	 */
	function _fnAjaxUpdateDraw ( settings, j )
	{
		// v1.10 uses camelCase variables, while 1.9 uses Hungarian notation.
		// Support both
		var compat = function ( old, modern ) {
			return j[old] !== undefined ? j[old] : j[modern];
		};
		...
		var data = _fnAjaxDataSrc( settings, j );
	...
	/**
	 * Get the data from the J data source to use for drawing a table. Using
	 * `_fnGetObjectDataFn` allows the data to be sourced from a property of the
	 * source object, or from a processing function.
	 *  @param {object} oSettings dataTables settings object
	 *  @param  {object} j Data source object / array from the server
	 *  @return {array} Array of data to use
	 */
	function _fnAjaxDataSrc ( oSettings, j )
	{
		...
		if ( dataSrc === 'data' ) {
			return j.aaData || j[dataSrc];
		}
	
		return dataSrc !== "" ?
			_fnGetObjectDataFn( dataSrc )( j ) :
			j;
	}
	...
	function _fnInitialise ( settings )
	{
		...
		if ( dataSrc != 'ssp' ) {
			...
			if ( dataSrc == 'ajax' ) {
				_fnBuildAjax( settings, [], function(j) {
					var aData = _fnAjaxDataSrc( settings, j );
	 ...
	 * Draw the table for the first time, adding all required features
	 *  @param {object} oSettings dataTables settings object
	 *  @param {object} [j] J from the server that completed the table, if using Ajax source
	 *    with client-side processing (optional)
	 *  @memberof DataTable#oApi
	 */
	function _fnInitComplete ( settings, j )
	{
		...
		_fnCallbackFire( settings, 'aoInitComplete', 'init', [settings, j] );
	}
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 * and sorting function (from oSort) in a certain direction. It's reaably complex to
	 ...
	 * shallow copy arrays. The rea we need to do this, is that we don't want to
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			if ( oInit.oLanguage.sUrl !== "" )
			{
				...
				$.getJ( oSettings.oLanguage.sUrl, null, function( j ) {
					_fnLanguageCompat( j );
					_fnCamelToHungarian( defaults.oLanguage, j );
					$.extend( true, oSettings.oLanguage, oInit.oLanguage, j );
	...
	/**
	 * Get the J response from the last Ajax request that DataTables made to the
	 * server. Note that this returns the J from the first table in the current
	 * context.
	 *
	 * @return {object} J received from the server.
	 */
	_api_register( 'ajax.j()', function () {
	...
	DataTable.defaults = {
		 ...
		 *   the J returned from the server, which can be manipulated as
		 ...
		 *
		 * @example
		 *   // Get J data from a file via Ajax.
		 ...
		 *
		 * @example
		 *   // Get J data from a file via Ajax, using `dataSrc` to change
		 ...
		 *
		 * @example
		 *   // Get J data from a file via Ajax, using `dataSrc` to read data
		 ...
		 *       "url": "data.j",
		 *       "dataSrc": function ( j ) {
		 *         for ( var i=0, ien=j.length ; i<ien ; i++ ) {
		 *           j[i][0] = '<a href="/message/'+j[i][0]+'>View message</a>';
		 *         }
		 *         return j;
		 *       }
		 *     }
		 ...
		 *       callback(
		 *         J.parse( localStorage.getItem('dataTablesData') )
		 *       );
		 *     }
		 *   } );
		 */
		 ...
		 *  @type function
		 *  @param {object} settings DataTables settings object
		 *  @param {object} j The J object request from the server - only
		...
		"oLanguage": {
			 ...
			 * It must store the URL of the language file, which is in a J format,
		 ...
		 * already have). Simply provide a url a J object can be obtained from.
		 *  @type string
		 *  @default null
		 *
		 *  @dtopt Options
	...
	DataTable.models.oSettings = {
		 ...
		 *       and the J string to save that has been thus far created. Returns
		 *       a J string to be inserted into a j object
		 *       (i.e. '"param": [ 0, 1, 2]')</li>
		...
		/**
		 * J returned from the server in the last Ajax request
		 *  @type object
		 *  @default undefined
		 */
		"j": undefined,
	 ...
	 * also aliased to `jQuery.fn.dataTableExt` for historic reas.
	 *  @namespace
	 ...
	 *  @param {event} e jQuery event object
	 *  @param {object} oSettings DataTables settings object
	 *  @param {object} j The state information to be saved
	 */
	 ...
	 *  @param {event} e jQuery event object
	 *  @param {object} oSettings DataTables settings object
	 *  @param {object} j The saved state information
	 */
	 ...
	 *  @param {event} e jQuery event object
	 *  @param {object} o DataTables settings object {@link DataTable.models.oSettings}
	 *  @param {object} j J returned from the server
	 *
	 *  @example
	 *     // Use a custom property returned from the server in another DOM element
	 *     $('#table').dataTable().on('xhr.dt', function (e, settings, j) {
	 ...
	 *     $('#table').dataTable().on('xhr.dt', function (e, settings, j) {
	 *       for ( var i=0, ien=j.aaData.length ; i<ien ; i++ ) {
	 *         j.aaData[i].sum = j.aaData[i].one + j.aaData[i].two;
	 *       }
	 *       // Note no return - manipulate the data directly in the J object.
	 *     } );
	 */
